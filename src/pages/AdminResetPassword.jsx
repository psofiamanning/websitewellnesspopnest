...
function AdminResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      const result = await adminResetPassword(token, password)
      if (result.success) {
        setSuccess(true)
      ...
      <div className="wellness-background min-h-screen flex items-center justify-center py-20">
        ...
        <div className="wellness-content relative z-10 w-full max-w-md mx-auto px-4">
          <div className="bg-white rounded-lg p-8 shadow-xl border-2" style={{ borderColor: '#E5B3B0' }}>
            ...
            <Link to="/admin/forgot-password" className="text-center block font-body hover:underline" style={{ color: '#B73D37' }}>
              Solicitar nuevo enlace
            </Link>
            <Link to="/admin/login" className="mt-4 text-center block text-sm font-body hover:underline" style={{ color: '#6B7280' }}>
      ...
      <div className="wellness-content relative z-10 w-full max-w-md mx-auto px-4">
        <div className="bg-white rounded-lg p-8 shadow-xl border-2" style={{ borderColor: '#E5B3B0' }}>
          ...
          <p className="text-body font-body text-center mb-8" style={{ color: '#6B7280' }}>
            Elige una contraseña de al menos 6 caracteres para el panel de administración
          </p>
          ...
          {success ? (
            <div className="mb-6 p-6 rounded-lg border-2" style={{ backgroundColor: '#f0fdf4', borderColor: '#86efac' }}>
              <p className="text-sm font-body text-gray-800">Contraseña actualizada. Redirigiendo al acceso de administrador...</p>
            </div>
          ) : (
